...
MyPalletizerBasic::MyPalletizerBasic() {
    for (auto& val : error_angles) val = -1000.0;
    for (auto& val : error_coords) val = -1000.0;

    for (auto val : printList) messages_map.insert(val);
}
...
void* MyPalletizerBasic::readData() {
    rFlushSerial();
    if (!checkHeader()) return nullptr;
    byte data_len[1];
    byte r_data_3[3], r_data_4[4], r_data_10[10];

    if (readSerial(data_len, 1) != 1) return nullptr;
...
MyPalletizerAngles MyPalletizerBasic::getAngles() {
    ...
    while (true) {
        ...
        if (tempPtr == nullptr)
            ...
            for (int i = 0; i < 4; ++i) angles[i] = pAngles->at(i);
            delete pAngles;
            return angles;
        }
    }
...
MyPalletizerCoords MyPalletizerBasic::getCoords() {
    ...
    while (true) {
        ...
        if (tempPtr == nullptr)
            ...
            for (int i = 0; i < 4; ++i) tempCoords[i] = pCoords->at(i);
            delete pCoords;
            return tempCoords;
        }
    }
...
MyPalletizerEncoders MyPalletizerBasic::getEncoders() {
    ...
    while (true) {
        ...
        if (tempPtr == nullptr)
            ...
            for (int i = 0; i < 4; ++i) encoders[i] = pEncoders->at(i);
            delete pEncoders;
            return encoders;
        }
    }
    ...
    while (true) {
        ...
        if (tempPtr == nullptr)
            ...
            std::map<int, std::string>::iterator iter =
                messages_map.find(message);
            char* pChar = (char*)iter->second.c_str();
            if (iter != messages_map.end()) Serial.println(pChar);
        }
    }
}
...
void MyPalletizerBasic::moveCCoords(
    MyPalletizerCoords end_coord, int radius, int direction,
    int speed)  // direction:  0-clockwise,1-anticlockwise
{
    // end_coord
...
void MyPalletizerBasic::moveCCoords(
    MyPalletizerCoords center_coord, int direction,
    int speed)  // direction:  0-clockwise,1-anticlockwise
{
    // middle_coord
